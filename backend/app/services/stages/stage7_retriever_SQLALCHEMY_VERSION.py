"""Stage 7: Knowledge Retriever for Copilot - FIXED VERSION

Classifies natural language queries and retrieves relevant lender data from the knowledge base.

FIXES APPLIED:
- Changed from db.fetch() to db.execute(text())
- Changed from positional params ($1, $2) to named params (:param_name)
- Fixed row result mapping using row._mapping
...
async def _retrieve_by_cibil(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # FIXED: Use text() and proper result handling
...
async def _retrieve_by_pincode(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    result = await db.execute(text(query), {"pincode": pincode})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_lender_details(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    query = """
        ...
        INNER JOIN lenders l ON lp.lender_id = l.id
        LEFT JOIN lender_pincodes lpc ON l.id = lpc.lender_id
        WHERE LOWER(l.lender_name) LIKE LOWER(CONCAT('%', :lender_name, '%'))
          AND lp.is_active = TRUE
          AND l.is_active = TRUE
        GROUP BY l.lender_name, lp.id, lp.product_name, lp.min_cibil_score,
                 lp.min_vintage_years, lp.min_turnover_annual, lp.max_ticket_size,
    ...
    # FIXED: Named parameter
    result = await db.execute(text(query), {"lender_name": lender_name})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_for_comparison(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # FIXED: Build LIKE conditions with named parameters
    like_conditions = []
    query_params = {}
...
async def _retrieve_by_vintage(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    query = """
        ...
        WHERE lp.is_active = TRUE
          AND l.is_active = TRUE
          AND lp.min_vintage_years <= :vintage_years
        ORDER BY lp.min_vintage_years ASC, l.lender_name
        LIMIT 20
    """

    # FIXED: Named parameter
    result = await db.execute(text(query), {"vintage_years": vintage_years})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_by_turnover(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    query = """
        ...
        WHERE lp.is_active = TRUE
          AND l.is_active = TRUE
          AND lp.min_turnover_annual <= :turnover
        ORDER BY lp.min_turnover_annual DESC, l.lender_name
        LIMIT 20
    """

    # FIXED: Named parameter
    result = await db.execute(text(query), {"turnover": turnover})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_by_entity_type(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    query = """
        ...
        WHERE lp.is_active = TRUE
          AND l.is_active = TRUE
          AND lp.eligible_entity_types @> :entity_types::jsonb
        ORDER BY l.lender_name, lp.product_name
        LIMIT 20
    """

    # FIXED: Named parameter with JSON
    result = await db.execute(text(query), {"entity_types": json.dumps([entity_type])})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_by_ticket_size(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    query = """
        ...
        WHERE lp.is_active = TRUE
          AND l.is_active = TRUE
          AND lp.max_ticket_size >= :ticket_size
        ORDER BY lp.max_ticket_size ASC, l.lender_name
        LIMIT 20
    """

    # FIXED: Named parameter
    result = await db.execute(text(query), {"ticket_size": ticket_size})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_by_requirement(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # IMPORTANT: This uses string formatting for column name, which is safe here
    # because 'requirement' comes from our predefined keyword dict, not user input
    query = f"""
        SELECT
            l.lender_name,
            lp.product_name,
        ...
        WHERE lp.is_active = TRUE
          AND l.is_active = TRUE
          AND lp.{requirement} = :value
        ORDER BY l.lender_name, lp.product_name
        LIMIT 20
    """

    # FIXED: Named parameter
    result = await db.execute(text(query), {"value": value})
    rows = result.fetchall()
    return [dict(row._mapping) for row in rows]
...
async def _retrieve_general(db) -> List[Dict[str, Any]]:
    ...
    # FIXED: No params needed for this query
