"""Stage 7: Knowledge Retriever for Copilot - PROPERLY FIXED FOR ASYNCPG

CRITICAL FIX: This version uses asyncpg methods (fetch, fetchrow) instead of
SQLAlchemy's text() and execute(). The get_db_session() returns an asyncpg
connection, NOT a SQLAlchemy session.
...
class QueryType(str, Enum):
    ...
    KNOWLEDGE = "knowledge"  # NEW: For definitions, explanations, general loan concepts
    GENERAL = "general"


# ═══════════════════════════════════════════════════════════════
# QUERY CLASSIFICATION (unchanged - this part works)
# ═══════════════════════════════════════════════════════════════
...
def classify_query(query: str) -> Tuple[QueryType, Dict[str, Any]]:
    ...
    # Check for KNOWLEDGE queries (definitions, explanations, concepts)
    knowledge_indicators = [
        'what is', 'what are', 'explain', 'define', 'tell me about',
        'difference between', 'meaning of', 'how does', 'why do',
        'advantages of', 'disadvantages of', 'benefits of', 'types of',
        'examples of', 'when to use', 'how to', 'can you explain'
    ]

    # Common loan terminology keywords
    loan_terms = [
        'od', 'overdraft', 'cc', 'cash credit', 'term loan', 'working capital',
        'foir', 'dscr', 'ltv', 'roi', 'emi', 'processing fee', 'prepayment',
        'moratorium', 'tenure', 'collateral', 'secured', 'unsecured',
        'cibil', 'credit score', 'bureau', 'enquiry', 'dpd', 'npa',
        'gst', 'itr', 'balance sheet', 'turnover', 'vintage',
        'proprietorship', 'partnership', 'pvt ltd', 'entity type',
        'business loan', 'gold loan', 'property loan', 'msme loan',
        'video kyc', 'fi', 'field investigation', 'sanction', 'disbursement',
        'nbfc', 'bank', 'fintech', 'dsa', 'connector'
    ]

    # Check if query is asking about a loan concept/term
    ...
    # Also treat single/double word queries as knowledge if they match loan terms
    word_count = len(query_lower.split())
    if word_count <= 3:
        for term in loan_terms:
            if term in query_lower and not any(k in query_lower for k in ['which', 'who', 'find', 'list', 'show']):
                is_knowledge_query = True
                break

    if is_knowledge_query:
        params['query_topic'] = query_lower
...
# ═══════════════════════════════════════════════════════════════
# DATA RETRIEVAL (FIXED FOR ASYNCPG)
# ═══════════════════════════════════════════════════════════════
...
async def _retrieve_by_cibil(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Build query with $1 positional parameter (asyncpg style)
    if operator == '<=':
    ...
    # PROPER FIX: Use asyncpg's fetch() method with positional parameters
...
async def _retrieve_for_comparison(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Build dynamic WHERE clause for multiple lenders
    like_conditions = ' OR '.join([f"LOWER(l.lender_name) LIKE LOWER(${i+1})" for i in range(len(lenders))])
    like_params = [f'%{lender}%' for lender in lenders]
    ...
    rows = await db.fetch(query, *like_params)
    return [dict(row) for row in rows]
...
async def _retrieve_by_requirement(db, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Note: requirement is from our predefined dict, so safe to use in SQL
    query = f"""
        SELECT
            l.lender_name,
            lp.product_name,
